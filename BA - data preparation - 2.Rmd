...
Foreign direct investment has the wrong data type

```{r}

data$foreign_investment <- as.numeric(gsub("−", "-",data$foreign_investment))
```
...
```{r}
mirror <- (max(data$foreign_investment) - min(data$foreign_investment)) / 2
...
Now, Luxembourg 2007 and Malta 2007 switched places
