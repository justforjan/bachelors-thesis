...
input_cols <- c(2,3,5,8)
output_cols <- c(4,6,7,9)

# only from 1990?
from_1990 <- "yes"

# only one year?
only_one_year <- 2010 # 0 means no
...
```{r}
filename <- paste(as.character(attempt), 
      "_", 
      orientation, 
      "_", 
      return_to_scale
...
if (from_1990 != "no") {
  filename <- paste(filename, "_", "from1990")
}

if (only_one_year != 0) {
  filename <- paste(filename, "_", as.character(only_one_year))  
}

if (only_one_country != "") {
  filename <- paste(filename, "_", only_one_country)    
}
