...
unique(final_8$country[final_8$women_in_parliament <= 0])
print("0 forest area:")
unique(final_8$country[final_8$forest_area <= 0])
```

Assign very low values where values are 0

```{r}
final_8[final_8 == 0] <- 0.0001
```
...
```{r}
final_8$dmu <- str_c(final_8$country, "_", final_8$year)
final_8$country <- NULL
final_8$year <- NULL
final_8 <- final_8 %>% relocate(dmu)
```
...
model_data <- read_data(final_8, ni=number_of_inputs, no=number_of_outputs, dmus = 1)
