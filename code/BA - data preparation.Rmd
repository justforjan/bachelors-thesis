...
```{r}
final_4$dmu <- str_c(final_4$country, "_", final_4$year)
final_4$country <- NULL
final_4$year <- NULL
final_4 <- final_4 %>% relocate(dmu)
```
...
model_data <- read_data(final_4, ni=number_of_inputs, no=number_of_outputs, dmus = 1)


result_4 <- model_basic(model_data, orientation = orientation, rts=return_to_scale)
...
```{r}
# Efficiencies
eff <- as.data.frame(efficiencies(result_4))
...
# get targets
targets <- as.data.frame(targets(result_4))
# add column for dmu names
...
data_dea_4 <- data_and_results %>% reduce(full_join, by="dmu")
```

Relocate the efficiencies column

```{r}
data_dea_4 <- data_dea_4 %>% relocate(efficiencies, .after = dmu)
```

Separate the DMU column into country and year but keep the dmu column

```{r}
data_dea_4 <- separate(data_dea_4, dmu, c("country", "year"), "_", remove=FALSE)
```

Add the continent to each DMU

```{r}
data_dea_4$continent <- countrycode(sourcevar = data_dea_4$country,
                               origin = "country.name",
...
result_3 <- model_basic(model_data, orientation = orientation, rts=return_to_scale)
...
```{r}
# Efficiencies
eff <- as.data.frame(efficiencies(result_3))
# add column for dmu names
eff$dmu <- rownames(eff)
# delete rownames
rownames(eff) <- NULL
...
# get targets
targets <- as.data.frame(targets(result_3))
# add column for dmu names
targets$dmu <- rownames(targets)
# delte rownames
rownames(targets) <- NULL
