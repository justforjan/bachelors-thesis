...
foreign_investment <- foreign_investment[ , !(names(foreign_investment) %in% drops)]
hdi <- hdi[ , !(names(hdi) %in% drops)]
...
foreign_investment<- foreign_investment %>% gather("year", "value", -country)
hdi <- hdi %>% gather("year", "value", -country)
```
...
# foreign_investment$value <- gsub("k", "e3", foreign_investment$value)
foreign_investment$value <- as.numeric(gsub("−", "-", foreign_investment$value))
...
foreign_investment$dmu <- str_c(foreign_investment$country, "_", foreign_investment$year)
hdi$dmu <- str_c(hdi$country, "_", hdi$year)
...
CO2$country <- NULL
murder$country <- NULL
foreign_investment$country <- NULL
hdi$country <- NULL
...
foreign_investment <- foreign_investment %>% relocate(dmu)
hdi <- hdi %>% relocate(dmu)
```
