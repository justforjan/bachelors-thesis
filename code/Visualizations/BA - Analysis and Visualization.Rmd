...
```{r}
cor(data_general[c("CO2", "efficiencies")])
```
...
```{r}
cor(data_general[c("foreign_investment", "efficiencies")])
```
...
```{r}
cor(data_general[c("material_footprint", "efficiencies")])
```

Small correlation (-0.3)

```{r}
cor(data_general[c("government_debt", "efficiencies")])
```
