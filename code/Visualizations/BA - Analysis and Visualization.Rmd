...
```{r}
cor(data_general[c("CO2", "efficiency_crs")])
```
...
```{r}
cor(data_general[c("foreign_investment", "efficiency_crs")])
```
...
```{r}
cor(data_general[c("material_footprint", "efficiency_crs")])
```

Small correlation (-0.3)

```{r}
cor(data_general[c("government_debt", "efficiency_crs")])
```

![](images/paste-3DFE937F.png)

The correlation between inputs and the efficiency does not correlate with the variance of the inputs.
...
names(oneonone)[length(names(oneonone))] <- "full_dea_crs"
