...
```{r}
MF_upperQuantile <- subset(data_crs, CO2 > as.numeric(quantile(data_crs$material_footprint)[4]))
MF_lowerQuantile <- subset(data_crs, CO2 <= as.numeric(quantile(data_crs$material_footprint)[2]))
...
```{r}
FDI_upperQuantile <- subset(data_crs, CO2 > as.numeric(quantile(data_crs$foreign_investment)[4]))
FDI_lowerQuantile <- subset(data_crs, CO2 <= as.numeric(quantile(data_crs$foreign_investment)[2]))
...
Slight negative correlation between eficiencies and HDI
